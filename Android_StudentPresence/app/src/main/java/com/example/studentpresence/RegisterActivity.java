            ...
            public void onClick(View view) {
                if (email_reg.getText().length() > 0 && password1.getText().length() > 0 && password2.getText().length() > 0 && studentNum.getText().length() > 0 && firstName.getText().length() > 0 && lastName.getText().length() > 0) { // Length Checker to see if every field has been filled in.
                    if (password1.getText().toString().equals(password2.getText().toString())) { // Check if Passwords match.
                        ...
                        final DatabaseReference studentRef = databaseRef.child("Students"); // Set reference to the child in the database called "Classes".
                        String studentNumString = studentNum.getText().toString();
                        studentRef.orderByChild("userStudentNum").equalTo(studentNumString).addValueEventListener(new ValueEventListener() {
                                @Override
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    System.out.println("LOADED" + dataSnapshot.getValue());
                                    try{numChecker = dataSnapshot.getValue().toString();}
                                    catch(Exception NullPointerException){numChecker = "1";}
                                    System.out.println("-=-=-=- SNAPSHOT CONTAINS: " + numChecker);
                                    if (numChecker.equals("1") || numChecker == null) {
                                    ...
                                    else {Toast.makeText(RegisterActivity.this, "Student Number already in use.", Toast.LENGTH_LONG).show();}
                                    }
                                @Override
                                public void onCancelled(DatabaseError databaseError) {
                                    System.out.println("The read failed: " + databaseError.getCode());
